 ...
 * License: CC0
 * Source: N/A
 * Description: Fast maximum bipartite matching algorithm.
 * Let the left and right partitions have size $n$, $m$ respectively. 
 ...
 * \texttt{btoa[i]} will be the match for vertex $i$ 
 * of the right partition, or $-1$ if it's not matched.
...
bool dfs(int a, int L, vvi& g, vi& btoa, vi& A, vi& B) {
  if (A[a] != L) return 0;
  A[a] = -1;
...
int hopcroftKarp(vvi& g, vi& btoa) {
  int res = 0;
