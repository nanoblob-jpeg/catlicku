/**
 * Author: Jerry Li
 * Description: (Array Merging)When doing DP on intervals: $a[i][j] = \min_{i \le k < j}(a[i][k] + a[k+1][j]) + f(i, j)$.
 * (Array Partition) For min cost to split an array of length n into m partitions.
 *  Sufficient criteria for this are if $f(b,c) \le f(a,d)$ and $f(a,c) + f(b,d) \le f(a,d) + f(b,c)$ for all $a \le b \le c \le d$.
 *  (i, j) are 0 indexed. k (if partition) is 1 indexed.
